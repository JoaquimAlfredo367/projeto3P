...
/* Reset de turno — preserva dailySummaries */
function clearAll() {
  if (!confirm('Limpar pedidos, clientes e eventos?\n\nO Histórico de Dias permanece salvo.')) return;
  ...
  openPainelDB().then(db => {
    // NÃO inclui 'dailySummaries' — histórico sobrevive ao reset normal
  ...
  dbSet('dbStatus', 'IndexedDB limpo (histórico de dias preservado)');
  renderAll();
  showToast('Dados do turno limpos. Histórico mantido.');
}

/* Reset total destrutivo — apaga tudo incluindo histórico */
function clearEverything() {
  if (!confirm('⚠ ATENÇÃO: Apagar TUDO incluindo o Histórico de Dias?\n\nEssa ação não pode ser desfeita.')) return;
  orders = []; clients = {}; revenue = 0; doneCnt = 0; newCliCnt = 0; seen.clear(); evCount = 0;
  try {
    localStorage.removeItem('fc_events');
    localStorage.removeItem('fc_trigger');
    localStorage.removeItem('dailySummaries');
  } catch(e) {}
  openPainelDB().then(db => {
    ['orders', 'clients', 'settings', 'seen', 'dailySummaries'].forEach(store => {
      try { db.transaction(store, 'readwrite').objectStore(store).clear(); } catch(e) {}
    });
  });
  dbSet('dbEvCount', 0); dbSet('dbLast', '—'); dbSet('dbLS', '—');
  dbSet('dbStatus', 'Reset total — todos os dados apagados');
  renderAll();
  showToast('Reset total concluído.');
}
